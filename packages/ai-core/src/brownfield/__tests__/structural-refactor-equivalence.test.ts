/**
 * Structural Refactoring Equivalence Tests — Aegis V2.3 Project 2 Phase 4
 */
...
describe("Structural Refactoring Equivalence Tests", () => {
  let testDir: string;
  let engine: StructuralTransformationEngine;

  beforeEach(() => {
    testDir = makeProject("aegis-equiv-eng-");
    writeFileSync(
  ...
  it("TEST 1: Cold == Warm deterministic planHash equivalence", () => {
    const coldPlan = engine.transformMoveSymbol({
      kind: "MOVE_SYMBOL",
      projectPath: testDir,
    ...
    const warmPlan = engine.transformMoveSymbol({
      kind: "MOVE_SYMBOL",
      projectPath: testDir,
      sourceFile: "src/services/taskService.ts",
      symbolName: "updateTask",
