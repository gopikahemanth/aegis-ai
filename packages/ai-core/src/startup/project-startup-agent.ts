  ...
  private applyDeterministicTsFixes(dir: string): { fixed: string[]; truncated: string[] } {
    ...
    for (const absPath of tsxFiles) {
      try {
        ...
        if ((rel.includes("Dashboard") || rel.includes("App.tsx") || rel.includes("Kanban")) && content.length < 350 && (content.includes("<div>Dashboard</div>") || content.includes("<div>App</div>") || !content.includes("<header"))) {
