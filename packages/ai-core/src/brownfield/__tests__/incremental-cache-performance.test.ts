/**
 * IncrementalCachePerformance Test Suite — Aegis V2.3 Project 2 Phase 2
 *
 * Tests:
 * - Proves reduction in AST parse count (O(1) vs O(N))
 * - Metric collection on cache hits and preserved nodes
 */
...
function makeProject(prefix: string) {
  ...
  for (let i = 1; i <= 10; i++) {
    writeFileSync(join(dir, "src", `module${i}.ts`), `export function fn${i}() { return ${i}; }\n`, "utf8");
  }
  return dir;
}

describe("IncrementalCachePerformance Tests", () => {
  let testDir: string;
  let cache: PersistentAstCache;

  beforeEach(() => {
    testDir = makeProject("aegis-perf-test-");
  ...
  it("TEST 1: In a 10-file repository, modifying 1 file parses only 1 file and preserves 9", () => {
    writeFileSync(join(testDir, "src", "module1.ts"), `export function fn1() { return 999; }\n`, "utf8");
    ...
    expect(result.metrics.graphNodesPreserved).toBe(9);
  });
});
